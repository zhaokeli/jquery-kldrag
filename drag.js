...
function getobj(dragblockobj,dragitem,da){
	...
	dragblockobj.find(dragitem).each(function(index, element) {
		...
		bo=(cury!==yyy && curx!==xxx  && absy<(cH/3) && absx<(cW/3));
		if(bo){
			if(yyy<cury){
				itemnum=index;
				info.status=1;
...
$.fn.kldrag = function(options, callback) { 
	...
	this.bind('mousemove',function(e){	
		if(data.mousedown){
			...
			var da={y:yy,x:xx,h:obj.outerHeight(),w:obj.outerWidth()};
			info=getobj(dragblock,opts.dragitem,da);
			if(info.status===1){
				$('.divspace').remove();
				str='<div class="divspace" style="margin:'+obj.css('margin')+';width:'+obj.outerWidth()+'px;height:'+obj.outerHeight()+'px;border:dashed 1px #f00;'+opts.spacestyle+'"></div>';
